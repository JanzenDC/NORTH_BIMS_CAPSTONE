    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (in_array($file['type'], $allowedTypes)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $targetFile)) {
                ...
                $sql = "UPDATE tblregistered_account SET image = '$fileName' WHERE id = $user_id";

                if ($conn->query($sql) === TRUE) {
                    // Update the session with the new image name
                    $_SESSION['user']['image'] = $fileName;

                    jsonResponse("success", "Profile picture updated successfully!", $fileName);
                } else {
        ...
        jsonResponse("success", "No new image uploaded. Using existing image.", $currentImage);
    }
}
