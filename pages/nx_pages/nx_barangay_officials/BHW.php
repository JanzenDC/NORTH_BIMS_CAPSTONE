...
<div class="p-3 w-full bg-white">
    <h1 class="text-3xl font-bold">Barangay Health Worker</h1>
    <hr>
    <button onclick="openModal('createModal')" class="bg-green-500 text-white px-4 py-2 rounded mb-4 mt-2">Add Barangay Health Worker</button>
...
<!-- MODALS SECTION -->
<!-- Create Barangay Health Worker Modal -->
<div id="createModal" class="modal fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        ...
        <h2 class="text-lg font-semibold mb-4">Create Barangay Health Worker</h2>
        <form id="createForm" enctype="multipart/form-data" onsubmit="event.preventDefault(); addRecord();">
            ...
            <input type="text" name="position" id="addPosition" placeholder="Position" class="block w-full mb-2 p-2 border rounded" value="BHW" required>
...
function editRecord(id) {
    ...
    $.get('nx_query/manage_bhw.php?action=get&id=' + id, function(response) {
        if (response.success) {
...
function deleteRecord(id) {
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: 'nx_query/manage_bhw.php?action=delete&id=' + id,
                type: 'DELETE',
...
function updateRecord(event) {
    ...
    $.ajax({
        url: 'nx_query/manage_bhw.php?action=update',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
...
function addRecord() {
    ...
    $.ajax({
        url: 'nx_query/manage_bhw.php?action=create',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
